...
public class TotalMark {
	...
	private void doInBackground() throws Exception {
		...
		for (int i = 1; i < picketEntities.size(); i++) {
			...
			result.add(picketEntity.getInformation() + ". Сумма баллов по горизонтали:" + totalMarkHor + ", оценка - "
					+ getTotalMark(totalMarkHor, 15, 30, 250) + ". Сумма баллов по вертикали:" + totalMarkVer
